...
const MAT_EQUIPMENT = [
  { id: 'mat', label: 'Mat', emoji: 'ð©' },
  { id: 'theraband', label: 'Theraband', emoji: 'ðï¸' },
  { id: 'magic-circle', label: 'Magic Circle', emoji: 'â­' },
  { id: 'small-ball', label: 'Small Ball', emoji: 'ðµ' },
  { id: 'foam-roller', label: 'Foam Roller', emoji: 'ðªµ' },
  { id: 'weights', label: 'Hand Weights', emoji: 'ðï¸' },
  { id: 'block', label: 'Yoga Block', emoji: 'ð§±' },
  { id: 'strap', label: 'Stretch Strap', emoji: 'ðª¢' },
]

const REFORMER_EQUIPMENT = [
  { id: 'reformer', label: 'Reformer', emoji: 'âï¸' },
  { id: 'box', label: 'Long/Short Box', emoji: 'ð¦' },
  { id: 'jump-board', label: 'Jump Board', emoji: 'ð¦' },
  { id: 'magic-circle', label: 'Magic Circle', emoji: 'â­' },
  { id: 'pole', label: 'Pole / Dowel', emoji: 'ðª' },
  { id: 'theraband', label: 'Theraband', emoji: 'ðï¸' },
  { id: 'weights', label: 'Hand Weights', emoji: 'ðï¸' },
  { id: 'platform-extender', label: 'Platform Extender', emoji: 'ð' },
]

const BLOCK_EMOJIS = ['ð','ð§','ðª','ð¥','ð§','â¡','ð','ð¯','ð','ð¿','â¨','ð¦','ð¸','ðµ','ð']
...
async function exportToPDF(className: string, setup: { format: 'mat' | 'reformer'; duration: number; level: string }, blocks: ClassBlock[], blockMovements: Record<string, Movement[]>, blockNotes: Record<string, string>, equipment: Set<string>) {
  ...
  doc.text(`${setup.format === 'mat' ? 'Mat Pilates' : 'Reformer Pilates'} Â· ${setup.duration} min Â· ${setup.level.charAt(0).toUpperCase() + setup.level.slice(1)}`, margin, 21)
  doc.setFontSize(8); doc.text(`Generated by Pilates Music Builder Â· ${new Date().toLocaleDateString()}`, pageW - margin, 21, { align: 'right' })
  y = 36
  ...
  if (equipmentList.length > 0) {
    ...
    doc.setFontSize(9); doc.setFont('helvetica', 'normal'); doc.setTextColor(...(DARK as [number, number, number])); doc.text(equipmentList.join('  Â·  '), margin, y); y += 8; drawLine()
  }
  for (const block of blocks) {
    ...
    doc.setFontSize(8); doc.setFont('helvetica', 'normal'); doc.text(`${bMins}:${bSec}${blockBpm ? ` Â· avg ${blockBpm} BPM` : ''} Â· ${block.songs.length} tracks`, pageW - margin - 4, y + 7, { align: 'right' }); y += 14
    if (movements.length > 0) { checkPage(10); doc.setFontSize(8); doc.setFont('helvetica', 'bold'); doc.setTextColor(...(GREEN as [number, number, number])); doc.text('Movements:', margin + 2, y); doc.setFont('helvetica', 'normal'); doc.setTextColor(...(GRAY as [number, number, number])); const movLines = doc.splitTextToSize(movements.map(m => m.name).join('  Â·  '), contentW - 30); doc.text(movLines, margin + 25, y); y += movLines.length * 4 + 3 }
  ...
  doc.setFontSize(7); doc.setTextColor(...(LIGHTGRAY as [number, number, number])); doc.text('Pilates Music Builder Â· pilates-music-builder.vercel.app', pageW / 2, pageH - 8, { align: 'center' })
  doc.save(`${className.replace(/[^a-z0-9]/gi, '_')}_class_plan.pdf`)
}

// âââ Energy Arc âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
    ...
    <div className="max-w-2xl mx-auto py-10 px-4">
            ...
            {savedClasses.map(c => (
              <div key={c.id} className="flex items-center gap-2 px-4 py-3 rounded-xl border border-cream-300 bg-white hover:border-sage-300 transition-all">
                <button onClick={() => onLoad(c)} className="flex-1 text-left">
                  ...
                  <p className="text-xs text-sage-400 mt-0.5">{c.format === 'mat' ? 'ð§ Mat' : 'âï¸ Reformer'} Â· {c.duration} min Â· {c.level} Â· {new Date(c.updated_at).toLocaleDateString()}</p>
                </button>
                ...
                <button onClick={() => onLoad(c)} className="shrink-0 text-sage-400 text-sm hover:text-sage-600">Load â</button>
              </div>
            ))}
          </div>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 gap-3">
          {([{ value: 'mat', label: 'Mat Pilates', emoji: 'ð§', desc: 'Floor-based, flowing movement' }, { value: 'reformer', label: 'Reformer Pilates', emoji: 'âï¸', desc: 'Machine-based, resistance work' }] as const).map(f => (
      ...
      <div className="mb-10">
        ...
        <div className="grid grid-cols-3 gap-2">
          {([{ value: 'beginner', label: 'Beginner', emoji: 'ð±' }, { value: 'intermediate', label: 'Intermediate', emoji: 'â­' }, { value: 'advanced', label: 'Advanced', emoji: 'ð' }] as const).map(l => (
      ...
      <button onClick={() => onStart(format, duration, level)} className="w-full py-4 bg-sage-500 hover:bg-sage-600 text-white font-bold text-base rounded-2xl transition-colors shadow-sm">Build Class Structure â</button>
    </div>
  )
}

// âââ Block Name Editor ââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
    ...
    <div className="flex items-center gap-1.5 min-w-0">
      ...
      {editing ? (
        ...
        <button onClick={e => { e.stopPropagation(); setEditing(true) }}
          ...
          {name}<span className="ml-1 opacity-0 group-hover:opacity-40 text-xs font-normal">âï¸</span>
        </button>
      )}
    </div>
  )
...
// âââ Custom Block BPM Picker ââââââââââââââââââââââââââââââââââââââââââââââââââ
    ...
    <div className="mb-5">
      ...
      <div className="mb-5 p-4 bg-sage-50 rounded-xl border border-sage-100">
        ...
        <p className="text-xs text-sage-400 mt-2 text-center">
          Songs filtered to <span className="font-semibold text-sage-600">{block.bpmMin}â{block.bpmMax} BPM</span>
        </p>
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-sage-400 mb-3">Browse or search â selecting movements auto-updates BPM range</p>
...
// âââ Template Movement Picker âââââââââââââââââââââââââââââââââââââââââââââââââ
function MovementPicker({ block, format, level, selectedMovements, onToggle }: {
  ...
  return (
    <div className="mb-5">
      <div className="flex items-center justify-between mb-3">
        <div><h4 className="text-sm font-semibold text-sage-700">Movements</h4><p className="text-xs text-sage-400">Select what you're teaching â songs will match</p></div>
...
// âââ Block Notes ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// âââ Block Card âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
        ...
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="flex-1 min-w-0" onClick={onActivate}>
            <div className="flex items-center gap-1.5 mb-1">
              ...
              {isFull && <span className="text-xs font-semibold px-1.5 py-0.5 rounded-full bg-green-100 text-green-700">â Full</span>}
              {isOver && <span className="text-xs font-semibold px-1.5 py-0.5 rounded-full bg-red-100 text-red-600">Over</span>}
              {hasNotes && <span className="text-xs">ð</span>}
            </div>
        ...
        <button onClick={onActivate} className="w-full text-left">
          <div className="flex items-center justify-between mb-2">
            <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${colors.badge}`}>
              {targetBpm ? `~${targetBpm} BPM` : `${block.bpmMin}â${block.bpmMax} BPM`}
            </span>
          ...
          <div className="flex items-center gap-2 mt-2" onClick={e => e.stopPropagation()}>
            ...
            {isCustomBlock(block.id) && (
              <>
                ...
                <span className="text-xs text-sage-300">â</span>
...
// âââ Sortable Block Card Wrapper ââââââââââââââââââââââââââââââââââââââââââââââ
...
// âââ Equipment Checklist ââââââââââââââââââââââââââââââââââââââââââââââââââââââ
        ...
        {items.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => onToggle(item.id)}
              ...
              {checked && <span className="ml-auto text-xs">â</span>}
            </button>
          )
        })}
      </div>
...
// âââ Summary Panel ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
      ...
      {allSongs.length > 0 && (
        <div className="space-y-2">
          <button onClick={onSave} disabled={saveStatus === 'saving'}
            ...
            {saveStatus === 'saving' ? 'â³ Saving...' : saveStatus === 'saved' ? 'â Saved!' : saveStatus === 'error' ? 'â Error â try again' : savedId ? 'ð¾ Update Saved Class' : 'ð¾ Save Class'}
          </button>
          <button onClick={onExportPDF} className="w-full py-2.5 bg-sage-700 hover:bg-sage-800 text-white font-semibold text-sm rounded-xl transition-colors">ð Export as PDF</button>
          <button onClick={onCopy} className="w-full py-2.5 bg-white hover:bg-sage-50 border border-sage-200 text-sage-700 font-semibold text-sm rounded-xl transition-colors">ð Copy Class Plan</button>
        </div>
      )}
    </div>
  )
...
// âââ Main ClassBuilder ââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
export default function ClassBuilder() {
  ...
  function copyFullClass() {
    ...
    const lines = [`${className}`, `${setup.format === 'mat' ? 'ð§ Mat' : 'âï¸ Reformer'} Pilates â ${setup.duration} min â ${setup.level.charAt(0).toUpperCase() + setup.level.slice(1)}`, '', ...(equipmentList ? [`ð¦ Equipment: ${equipmentList}`, ''] : []), ...blocks.flatMap(b => { const movements = blockMovements[b.id] ?? []; const note = blockNotes[b.id] ?? ''; const bSecs = b.songs.reduce((a, s) => a + (s.duration ?? 0), 0); const bMins = Math.floor(bSecs / 60); const bSec = String(bSecs % 60).padStart(2, '0'); return [`${b.emoji} ${b.name} (${bMins}:${bSec})`, ...(movements.length > 0 ? [`   Movements: ${movements.map(m => m.name).join(', ')}`] : []), ...(note ? [`   Notes: ${note}`] : []), ...b.songs.map((s, i) => { const dur = s.duration ?? 0; const m = Math.floor(dur / 60); const sec = String(dur % 60).padStart(2, '0'); return `   ${i + 1}. ${s.title ?? s.name} â ${s.artist} (${s.bpm} BPM, ${m}:${sec})` }), ''] })]
        ...
        {activeBlock && setup ? (
          <>
            <div className="bg-cream-50 border border-cream-200 rounded-2xl p-5 mb-5">
              {isCustomBlock(activeBlock.id) ? (
                ...
                <>
                  ...
                  {activeMovements.length === 0 && <p className="text-xs text-sage-400 italic mb-5">No movements selected â showing songs in the {activeBlock.bpmMin}â{activeBlock.bpmMax} BPM range. Select movements to refine.</p>}
                </>
              )}
            ...
            <div className="flex items-center gap-3 mb-4 flex-wrap">
              <div>
                ...
                <p className="text-sm text-sage-400">
                  {activeBpmRange[0]}â{activeBpmRange[1]} BPM
                  {!isCustomBlock(activeBlock.id) && activeMovements.length > 0 ? ` Â· based on ${activeMovements.length} movement${activeMovements.length > 1 ? 's' : ''}` : ''}
                </p>
              </div>
                ...
                {songSource === 'library' && (
                  <>
                    <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="px-3 py-2 text-sm bg-white border border-cream-300 rounded-xl text-sage-700 focus:outline-none focus:border-sage-400">
                      ...
                      <option value="bpm-asc">BPM: Low â High</option>
                      <option value="bpm-desc">BPM: High â Low</option>
            ...
            {songSource === 'spotify' ? (
              ...
              <div className="text-center py-16"><div className="text-4xl mb-3">ð</div><p className="text-sage-500">No songs found in {activeBpmRange[0]}â{activeBpmRange[1]} BPM range</p></div>
            ) : (
              <>
