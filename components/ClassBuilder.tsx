...
async function exportToPDF(
  className: string,
  setup: { format: 'mat' | 'reformer'; duration: number; level: string },
  blocks: ClassBlock[],
  ...
  // Colors
  const GREEN = [76, 130, 76]
  const DARK = [30, 50, 30]
  const GRAY = [100, 110, 100]
  const LIGHTGRAY = [220, 228, 220]
  ...
  doc.text(className, margin, 13)
  doc.setFontSize(9)
  doc.setFont('helvetica', 'normal')
  const subtitle = `${setup.format === 'mat' ? 'Mat Pilates' : 'Reformer Pilates'} · ${setup.duration} min · ${setup.level.charAt(0).toUpperCase() + setup.level.slice(1)}`
  doc.text(subtitle, margin, 21)
  doc.setFontSize(8)
  doc.text(`Generated by Pilates Music Builder · ${new Date().toLocaleDateString()}`, pageW - margin, 21, { align: 'right' })
  y = 36
  ...
  if (equipmentList.length > 0) {
    ...
    doc.text('EQUIPMENT NEEDED', margin, y)
    y += 5
    doc.setFontSize(9)
    doc.setFont('helvetica', 'normal')
    doc.setTextColor(...(DARK as [number, number, number]))
    doc.text(equipmentList.join('  ·  '), margin, y)
    y += 8
    drawLine()
  }
  ...
  for (const block of blocks) {
    const movements = blockMovements[block.id] ?? []
    const note = blockNotes[block.id] ?? ''
    ...
    const songCount = block.songs.length
    const blockBpm = songCount > 0 ? Math.round(block.songs.reduce((a, s) => a + s.bpm, 0) / songCount) : null
    ...
    doc.setFontSize(10)
    doc.setFont('helvetica', 'bold')
    doc.text(`${block.name}`, margin + 4, y + 7)
    doc.setFontSize(8)
    doc.setFont('helvetica', 'normal')
    const blockInfo = `${bMins}:${bSec}${blockBpm ? `  ·  avg ${blockBpm} BPM` : ''}  ·  ${songCount} tracks`
    doc.text(blockInfo, pageW - margin - 4, y + 7, { align: 'right' })
    y += 14
    ...
    if (movements.length > 0) {
      ...
      doc.setFont('helvetica', 'bold')
      doc.setTextColor(...(GREEN as [number, number, number]))
      doc.text('Movements:', margin + 2, y)
      doc.setFont('helvetica', 'normal')
      doc.setTextColor(...(GRAY as [number, number, number]))
      const movText = movements.map(m => m.name).join('  ·  ')
      const movLines = doc.splitTextToSize(movText, contentW - 30)
      doc.text(movLines, margin + 25, y)
      y += movLines.length * 4 + 3
    }
    ...
    if (note.trim()) {
      ...
      doc.setTextColor(...(GRAY as [number, number, number]))
      const noteLines = doc.splitTextToSize(note, contentW - 20)
    ...
    if (block.songs.length === 0) {
      ...
      doc.text('No songs added to this block', margin + 4, y)
      y += 6
    } else {
      for (let i = 0; i < block.songs.length; i++) {
        const song = block.songs[i]
        ...
        doc.text(title.length > 35 ? title.substring(0, 35) + '...' : title, margin + 8, y + 2)

        doc.setFont('helvetica', 'normal')
        doc.setTextColor(...(GRAY as [number, number, number]))
        doc.text(song.artist, margin + 85, y + 2)

        doc.setFont('helvetica', 'bold')
        doc.setTextColor(...(GREEN as [number, number, number]))
        doc.text(`${song.bpm} BPM`, pageW - margin - 20, y + 2)

        doc.setFont('helvetica', 'normal')
        doc.setTextColor(...(GRAY as [number, number, number]))
        doc.text(`${m}:${s}`, pageW - margin - 4, y + 2, { align: 'right' })
  ...
  doc.setFontSize(7)
  doc.setTextColor(...(LIGHTGRAY as [number, number, number]))
  doc.text('Pilates Music Builder · pilates-music-builder.vercel.app', pageW / 2, pageH - 8, { align: 'center' })
...
function MovementPicker({ block, format, level, selectedMovements, onToggle }: {
  block: ClassBlock; format: 'mat' | 'reformer'
...
function BlockNotes({ blockId, value, onChange }: {
  blockId: string; value: string; onChange: (blockId: string, note: string) => void
}) {
  return (
    <div className="mb-5">
...
function SummaryPanel({ blocks, blockMovements, targetDuration, className, savedId, saveStatus, onSave, onCopy, onExportPDF, onReset }: {
  blocks: ClassBlock[]; blockMovements: Record<string, Movement[]>
  targetDuration: number; className: string; savedId: string | null
      ...
      {allSongs.length > 0 && (
        <div className="space-y-2">
          ...
          <button onClick={onExportPDF}
            className="w-full py-2.5 bg-sage-700 hover:bg-sage-800 text-white font-semibold text-sm rounded-xl transition-colors">
            📄 Export as PDF
          </button>
...
export default function ClassBuilder() {
  ...
  async function handleSave() {
    ...
    const payload = {
      id: savedId, name: className, format: setup?.format,
      duration: setup?.duration, level: setup?.level,
      data: { blocks, blockMovements, blockNotes, equipment: Array.from(equipment) },
    }
    try {
  ...
  async function handleExportPDF() {
    if (!setup) return
    try {
      await exportToPDF(className, setup, blocks, blockMovements, blockNotes, equipment)
    } catch (e) {
  ...
  if (!setup) return (
    <SetupScreen onStart={handleStart} savedClasses={savedClasses} onLoad={handleLoad} onDelete={handleDelete}/>
  )
    ...
    <div className="flex gap-6">
      <div className="w-80 shrink-0 space-y-3">
        ...
        <SummaryPanel blocks={blocks} blockMovements={blockMovements} targetDuration={setup.duration}
          className={className} savedId={savedId} saveStatus={saveStatus}
          onSave={handleSave} onCopy={copyFullClass} onExportPDF={handleExportPDF}
